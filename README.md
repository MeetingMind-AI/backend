...
- **Caching & Brokers:** Redis (asynchronous client with 60s TTL caching)
...
- `GET /api/system/status`
  - Inspects real-time health across all services: Ollama (deployment type: host vs docker, VRAM allocation, loaded models, latency), PostgreSQL connectivity, Redis broker/cache, Qdrant vector store, and STT gateway.
- `GET /health`
...
- `PATCH /api/teams/{team_id}/members/{user_id}` — Update team member `role` (`scrum_master`, `product_manager`, `team_member`, `admin`) and `notification_preferences` (tag filter array).
...
- `GET /api/teams/{team_id}/prompts` — Fetch all 8 persona prompt templates with team overrides or defaults.
- `PUT /api/teams/{team_id}/prompts/{prompt_key}` — Set custom prompt override for specified persona.
...
- `POST /api/meetings/start` — Launch Vexa bot (`platform`, `native_id`, `team_id`, optional `passcode`).
...
- `POST /api/meetings/{meeting_id}/resummarize` — Re-run full final multi-agent debate and summary generation from current transcript (admin/owner only). Cancels any active task and resets state.
...
- `PATCH /api/meetings/{meeting_id}` — Rename meeting title and/or update `meeting_type` (e.g. `general`, `sprint_planning`, `retrospective`, `standup`).
...
- `WS /api/ws/ingest/{meeting_id}` — WebSocket stream broadcasting real-time transcript chunks, insights, action proposals, and multi-agent thoughts to clients.
...
- `POST /api/meetings/{meeting_id}/actions` — Manually create an action item (`action_type`, `content`, optional `assignee_id`, `status`, `tags`).
...
| `team_memberships` | `id`, `user_id` (FK), `team_id` (FK), `joined_at`, `role`, `notification_preferences` | User membership, Agile role (`scrum_master`, `product_manager`, `team_member`, `admin`), and notification tag filter preferences. |
...
| `agent_actions` | `id`, `meeting_id` (FK), `assignee_id` (FK), `agent_role`, `action_type`, `content`, `status`, `tags` | Action items (`parking_lot`, `to_do`, `to_schedule`, `blocker`), approval status, and assignee. |
...
| `team_prompt_configs` | `id`, `team_id` (FK), `prompt_key`, `prompt_text`, `updated_at` | Per-team persona prompt overrides. |
