...
REALTIME_SCRUM_MASTER_PROMPT = (
    ...
    "4. 'parking_lot': Triggers whenever a topic is deferred, tabled, parked, or moved offline, or when speakers enter a deep technical debate / tangent during a standup that should be parked offline (e.g., 'Let\\'s table this discussion for later', 'Park this topic for the next sprint', deep architectural debate during a standup). proposal.type MUST be 'parking_lot'.\n"
    "5. 'blocker': Triggers whenever an impediment, dependency, or unresolved hurdle is articulated (e.g., 'I am blocked by missing API keys', 'We cannot deploy because the build failed'). proposal.type MUST be 'blocker'.\n"
